# @hishpr/architecto-consequatur

![CI](https://github.com/hishpr/architecto-consequatur/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@hishpr/architecto-consequatur.svg?style=flat)](https://www.npmjs.com/package/@hishpr/architecto-consequatur)
...
| --------------- | -------------------------------------------------------------------- |
| `^4.x`          | This branch                                                          |
| `^3.x`          | [`v5.x`](https://github.com/hishpr/architecto-consequatur/tree/v5.x)         |
| `^2.x`          | [`2.x`](https://github.com/hishpr/architecto-consequatur/tree/2.x)          |
| `^1.11.x`       | [`1.x`](https://github.com/hishpr/architecto-consequatur/tree/1.x)          |

## Install

`npm i @hishpr/architecto-consequatur`
...
fastify.register(require('@hishpr/architecto-consequatur'), {
...
const fastifyStatic = require('@hishpr/architecto-consequatur')
const path = require('node:path')
// first plugin
...
If set to `true`, `@hishpr/architecto-consequatur` redirects to the directory with a trailing slash.
...
If set to `true`, `@hishpr/architecto-consequatur` adds a wildcard route to serve files.
If set to `false`, `@hishpr/architecto-consequatur` globs the filesystem for all defined
...
disable this, pass the option `{ decorateReply: false }`.  If @hishpr/architecto-consequatur is
...
When registering `@hishpr/architecto-consequatur` within an encapsulated context, the `wildcard` option may need to be set to `false` in order to support index resolution and nested not-found-handler:

```js
const app = require('fastify')();

app.register((childContext, _, done) => {
    childContext.register(require('@hishpr/architecto-consequatur'), {
